...
>
	<div class="flex items-center gap-3">
        <Checkbox.Root class="group/check text-sm font-medium size-7" onclick={(e) => e.stopPropagation()}>
            {#snippet children({ checked })}
                <div>
                    <!-- Initial is hidden if checked or if it is hovered -->
                    <span class="{checked ? 'hidden' : 'block group-hover/check:hidden'}">
                        {initial}
                    </span>

                    <!-- Checkmark is shown if checked OR if the initial is hovered -->
                    <div class="{checked ? 'block' : 'hidden group-hover/check:block'}">
	...
	<div class="w-48 shrink-0 truncate text-sm {isUnread ? 'text-neutral-100 font-semibold' : 'text-neutral-400 font-normal'}">
		{message.sender}
	</div>

	<div class="flex-1 truncate text-sm {isUnread ? 'text-neutral-200 font-medium' : 'text-neutral-500'}">
		{message.subject}
	</div>
